...
| Synonym breadth 4-6 (2026-09) | Extra terms cost ~nothing in the fan-out itself (21/21 queries ok in the same wave count) but scale the pool, probes, and star enrichment with term novelty: 4-term fan-out ~8s / 329 pkgs / 1 probe / 31 repos vs 6-term with two adjacent topics ~20s / 476 pkgs / 12 probes / 61 repos. Guidance raised 2-4 → 4-6 forms, restricted to close synonyms (morphological variants + hyphenated compounds); adjacent topics are a separate fan-out, not more synonyms |
| Presentation is markdown, not ANSI (2026-09) | Skill output is markdown-rendered chat: ANSI codes are client-dependent (stripped or literal garbage) and cost tokens, so color instructions cannot work. Readability comes from contract structure instead — tier-grouped markdown tables with bold intent matches — and from hiding the script's internal query-hits column, which is routing metadata, not user information. Script-side ANSI would only affect direct human TTY runs, never agent-captured output; rejected as out of scope |
