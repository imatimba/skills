...
description: >-
  Write and debug userscripts for browser automation, page modification, and web
  enhancement across managers (Tampermonkey, Greasemonkey, Violentmonkey). Use
  whenever the user mentions userscripts/Tampermonkey/Greasemonkey/Violentmonkey
  or wants a script running on a website, even implicitly. Also trigger for:
  injecting JS/CSS, modifying page behavior, hiding elements, form auto-fill,
  scraping data, request interception, SPA URL changes, keyboard shortcuts, tab
  audio control, TypeScript userscripts. Covers header tags (@match, @grant,
  @require, @run-in), GM_* sync APIs, GM.* promise APIs (preferred), batch
  storage (v5.3+), binary data (v5.4+), @types/tampermonkey, sandboxing,
  cross-browser compat. Do NOT use for Selenium/Puppeteer automation, browser
...
Expert guidance for writing userscripts that work across userscript managers (Tampermonkey, Greasemonkey, Violentmonkey) - browser scripts that modify web pages, automate tasks, and enhance browsing experience.
...
**TypeScript:** Add `@types/tampermonkey` (npm package name; the namespace is `Tampermonkey` but the type definitions work for any userscript manager that implements the standard `GM_*`/`GM.*` APIs) for full type safety. See [typescript.md](references/typescript.md).
...
| `@grant`       | Situational | API permissions (use `none` for no GM\_\* APIs)                  | `@grant GM_setValue`                |
...
- **TypeScript Userscripts** - Type-safe scripts with `@types/tampermonkey` (npm package name). See [typescript.md](references/typescript.md)
...
// @require tampermonkey://vendor/jquery.js         // Tampermonkey-specific vendor URL protocol (other managers use @require with a regular URL instead)
...
```bash
npm install --save-dev @types/tampermonkey  # npm package name; global `Tampermonkey` TypeScript namespace
```
...
(async () => {
  ...
  const info: Tampermonkey.ScriptInfo = GM_info; // `Tampermonkey` is the global TypeScript namespace exported by @types/tampermonkey
...
## What Userscripts Cannot Do

Userscripts (regardless of manager) have limitations:
...
- **Inject without permission** - Tampermonkey v5.4.1+ (a specific manager version) requires users to grant injection permission per-site or globally; other managers handle injection differently or have no such requirement. Scripts cannot bypass this requirement.
...
3. **Installation** - "Copy/paste into your userscript manager's dashboard" (e.g. Tampermonkey Dashboard) or "Save as .user.js"
...
| **API**                                                         |                                                                                 |
| [api-sync.md](references/api-sync.md)                           | GM\_\* synchronous function reference (callback-based)                          |
...
| [typescript.md](references/typescript.md)                       | TypeScript project setup with @types/tampermonkey (npm package), bundler config |
