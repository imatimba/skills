...
### happy-dom vs jsdom trade-offs (verified 2026-08-25 — vitest.dev/guide/environment, github.com/capricorn86/happy-dom/wiki/Setup-as-Test-Environment)

`vitest.dev/guide/environment` notes `happy-dom` is faster but lacks some APIs — prefer `jsdom` when you need fuller DOM fidelity. For timers, happy-dom wiki "Setup as Test Environment" (verified 2026-08-25 — github.com/capricorn86/happy-dom/wiki/Setup-as-Test-Environment) documents that `happyDOM.waitUntilComplete()` and timer methods need rebinding via `PropertySymbol` workaround — see wiki snippet. The wiki notes "Hopefully this can be fixed soon" — treat the workaround as version-sensitive.

### Jest alternative (verified 2026-08-25 — github.com/capricorn86/happy-dom/wiki/Setup-as-Test-Environment)
...
### Coverage and CI (verified 2026-08-25 — vitest.dev/guide/coverage, vitest.dev/guide/cli)
...
### Isolation helpers (verified 2026-08-25 — github.com/melusc/mock-violentmonkey)
...
- Sandbox isolation (`@grant none` disables sandbox; otherwise `unsafeWindow` is a wrapper; Firefox `wrappedJSObject`/`cloneInto`/`exportFunction`) — manager-owned, smoke-test only (`violentmonkey.github.io/api/gm` and `/posts/inject-into-context/`, verified 2026-08-25 — violentmonkey.github.io/api/gm, violentmonkey.github.io/posts/inject-into-context/)
